...
class CBAM(nn.Module):
    def __init__(self, channels, reduction=16):
        ...
        # 空间注意力机制
        self.conv1 = nn.Conv3d(channels, 1, kernel_size=7, padding=3, bias=False)
    ...
    def forward(self, x):
        ...
        # 空间注意力机制
        spatial_attention = self.sigmoid(self.conv1(x) * channel_attention)
...
def deep_learning(model, trainloader, epochs, print_every, criterion, optimizer):
    ...
    # model = model.cuda()
    # # model.to('cuda:0')
    model = nn.DataParallel(model, device_ids=gpus, output_device=gpus[0]).to('cuda:0')
...
np.save("X_32_AlexNet_cbam8",X_3_32_2)
np.save("train_loss_32_AlexNet_cbam8",train_loss_3_32_2)
np.save("valid_loss_32_AlexNet_cbam8",valid_loss_3_32_2)
np.save("valid_accc_32_AlexNet_cbam8",valid_accc_3_32_2)

path_state_dict = "./model_state_dict_32_AlexNet_cbam8.pkl"
