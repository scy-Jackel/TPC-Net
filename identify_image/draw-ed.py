...
y2_=np.load("model/valid_accc_32_AlexNet_xiugai_32.npy")
y3_=np.load("model/valid_accc_32_AlexNet_cbam4.npy")
y4_=np.load("model/valid_accc_32_AlexNet_512_2.npy")
x=[]
# y1=[]
y2=[]
y3=[]
...
for i in range(y2_.shape[0]):
    if (i == 0):
        ...
        if(flag==1):
            y2.append(max(y2_[i], y2_[i + 1]))
            flag=0
        else:
            flag=1
flag=1
for i in range(y3_.shape[0]):
    if (i == 0):
        y3.append(y3_[i])
...
plt.plot(x, y2, c=(118.0/255,218.0/255,145.0/255),label="ouAlexNet")
plt.plot(x, y3, c=(248.0/255,149.0/255,136.0/255),label="EfficientNet")
# plt.plot(x, y4, c=(248.0/255,203.0/255,127.0/255),label="ouAlexNet1/4")
plt.legend(loc='best')
...
plt.ylabel("acc", fontdict={'size': 24})
