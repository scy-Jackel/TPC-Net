...
with os.scandir(path) as it:
    ...
    for iiiii in it:
        ...
        for iii in range(0,len(res)):
            ...
            current=int(res[iii])
            sum_len=0.0
            xx=yy=zz=px=py=pz=tmp1=tmp2-1.0

            while(sum_len<rannum):
                xx = result1.loc[current]['x']
                yy = result1.loc[current]['y']
                zz = result1.loc[current]['z']
                now = result1.loc[current]['parent']
                px = result1.loc[int(now)]['x']
                py = result1.loc[int(now)]['y']
                pz = result1.loc[int(now)]['z']
                tmp1=math.pow(xx-px,2)+math.pow(yy-py,2)+math.pow(zz-pz,2)
                tmp2=math.sqrt(tmp1)
                sum_len+=tmp2
                current=int(now)

            rannum=sum_len-rannum

            rr_x = round(px + 1.0 * rannum / tmp2 * (xx - px))
            rr_y = round(py + 1.0 * rannum / tmp2 * (yy - py))
            rr_z = round(pz + 1.0 * rannum / tmp2 * (zz - pz))
