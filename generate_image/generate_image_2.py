...
with os.scandir(path) as it:
    ...
    for iiiii in it:
        ...
        minn=len(res)
        sss=1
        for iii in range(sss,minn):
            print("开始更改第"+str(iii+1)+"个终端点图像")
            res_z=[]
            res_x=[]
            res_y=[]
            ...
            #遍历并MIP
            res_max_xy = np.zeros((50, 50, 3))
            res_max_zy = np.zeros((50, 50, 3))
            res_max_zx = np.zeros((50, 50, 3))
            ...
            for pic in str_z:
                ...
                zzz=zzz+1
            res_max_xy = np.max(res_rgb, axis=0)
            res_max_zy = np.max(res_rgb, axis=1)
            res_max_zx = np.max(res_rgb, axis=2)

            res_max_xy = res_max_xy / 4095.0 * 255.0
            res_max_zy = res_max_zy / 4095.0 * 255.0
            res_max_zx = res_max_zx / 4095.0 * 255.0
            res_rgb = res_rgb / 4095.0 * 255.0

            name1 = "data1-" + str(final_num) + '-' + str(iii + 1) + ".tif"
            tifffile.imsave(name1, res_rgb)

            im1_xy = Image.fromarray(res_max_xy)
            name1_xy = "data1-" + str(final_num) + '-' + str(iii + 1) + ".tif"
            im1_xy.save(name1_xy)
            im1_zy = Image.fromarray(res_max_zy)
            name1_zy = "data1-" + str(final_num) + '-' + str(iii + 1) + ".tif"
            im1_zy.save(name1_zy)
            im1_zx = Image.fromarray(res_max_zx)
            name1_zx = "data1-" + str(final_num) + '-' + str(iii + 1) + ".tif"
            im1_zx.save(name1_zx)
            print("data1生成完毕")

            img = tifffile.imread(name1)
            range_all = img.max() - img.min()
            img = (img - img.min()) / range_all * 255.0

            res_max_xy = np.max(img, axis=0)
            res_max_zy = np.max(img, axis=1)
            res_max_zx = np.max(img, axis=2)

            name2 = "data2-" + str(final_num) + '-' + str(iii + 1) + ".tif"
            tifffile.imsave(name2, img)

            im1_xy = Image.fromarray(res_max_xy)
            name1_xy = "data2-" + str(final_num) + '-' + str(iii + 1) + ".tif"
            im1_xy.save(name1_xy)
            im1_zy = Image.fromarray(res_max_zy)
            name1_zy = "data2-" + str(final_num) + '-' + str(iii + 1) + ".tif"
            im1_zy.save(name1_zy)
            im1_zx = Image.fromarray(res_max_zx)
            name1_zx = "data2-" + str(final_num) + '-' + str(iii + 1) + ".tif"
            im1_zx.save(name1_zx)
            print("data2生成完毕")
            ...
            name8_xy = "data-3-" + str(final_num) + '-' + str(iii + 1) + "-xy.tif"
            img_xy = Image.fromarray(np.uint8(img_res_xy))
            img_xy.save(name8_xy)
            name8_zy = "data-3-" + str(final_num) + '-' + str(iii + 1) + "-zy.tif"
            img_zy = Image.fromarray(np.uint8(img_res_zy))
            img_zy.save(name8_zy)
            name8_zx = "data-3-" + str(final_num) + '-' + str(iii + 1) + "-zx.tif"
            ...
            name4 = "data-4-" + str(final_num) + '-' + str(iii + 1) + ".tif"
            tifffile.imsave(name4, img)
            ...
            name8_xy = "data-4-" + str(final_num) + '-' + str(iii + 1) + "-xy.tif"
            img_xy = Image.fromarray(np.uint8(img_res_xy))
            img_xy.save(name8_xy)
            name8_zy = "data-4-" + str(final_num) + '-' + str(iii + 1) + "-zy.tif"
            img_zy = Image.fromarray(np.uint8(img_res_zy))
            img_zy.save(name8_zy)
            name8_zx = "data-4-" + str(final_num) + '-' + str(iii + 1) + "-zx.tif"
            img_zx = Image.fromarray(np.uint8(img_res_zx))
            img_zx.save(name8_zx)
            print("data4映射完毕")
